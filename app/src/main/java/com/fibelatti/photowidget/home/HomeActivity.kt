...
class HomeActivity : AppCompatActivity() {
    ...
    private fun showSettings() {
        val activity = this
        ComposeBottomSheetDialog(this) {
            SettingsScreen(
                onDefaultsClick = {
                    dismiss()
                    activity.showDefaults()
                },
                onDataSaverClick = {
                    dismiss()
                    activity.showDataSaverPicker()
                },
                onAppearanceClick = {
                    dismiss()
                    activity.showAppearancePicker()
                },
                onColorsClick = {
                    dismiss()
                    activity.showAppColorsPicker()
                },
                onAppLanguageClick = {
                    dismiss()
                    activity.showTranslationsDialog()
                },
                onBackupClick = {
                    dismiss()
                    startActivity(PhotoWidgetBackupActivity.newIntent(activity))
                },
                onSendFeedbackClick = {
                    dismiss()
                    activity.showHelp()
                ...
                onViewLicensesClick = {
                    dismiss()
                    activity.viewOpenSourceLicenses()
                },
            )
        }.show()
    }
    ...
    private fun createNewWidget(aspectRatio: PhotoWidgetAspectRatio) {
        ...
        val intent: Intent = preparedIntent?.apply { this.aspectRatio = aspectRatio }
            ?: PhotoWidgetConfigureActivity.newWidgetIntent(
                context = this, 
